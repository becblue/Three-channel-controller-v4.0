...
│   │       ├── temperature.h         # 温度检测控制模块（包含NTC查表）
...
│           ├── temperature.c         # 温度检测与风扇控制（ADC采集、查表转换、166点精度）
...
- 包含7个核心功能模块：
  ...
  4. `temperature` - 温度采集与风扇控制（含NTC查表）
  5. `alarm_output` - 多级报警输出
  6. `oled_display` - 人机界面显示
  7. `self_test` - 系统自检流程
...
| **阶段3** | 驱动层-NTC | ~~已整合到temperature.c~~ | - | - |
| **阶段4** | 驱动层-温度 | temperature, adc, dma | common_def, usart | 2小时 |
...
#### **交付文件**
- `Core/Inc/temperature.h`（包含NTC查表定义）
- `Core/Src/temperature.c`（包含166点查表数据）
...
#### **依赖关系**
- 依赖：`common_def.h`、`usart.h`、`adc.h`、`tim.h`
...
#### **⚠️ 已知问题（当前版本）**
- **外部存储 / 数据日志**：KEY1 长按 1 秒触发串口 dump 时，`Total: N records` 显示正常，但**无法正确读出并显示记录内容**（dump 仅输出 START/END，中间无记录行）。写入、格式化与 record_count 均正常，问题出在 W25Q Flash 读取或 SPI 时序环节，待后续修复。
